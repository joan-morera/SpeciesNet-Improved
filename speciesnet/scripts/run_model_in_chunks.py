...
def _execute_and_print(cmd,
    ...
    output = []
    try:
        for s in _execute(cmd,encoding=encoding,errors=errors,env=env,verbose=verbose):
            output.append(s)
            if print_output:
...
def _create_argument_list_with_exceptions(all_args,exclude_args):
    ...
    for i_arg, arg in enumerate(all_args):
        ...
        for exclude in exclude_args:
            ...
            if (arg == exclude) and (i_arg < len(all_args) - 1) and \
                (not all_args[i_arg+1].startswith('-')):
                should_exclude = True
...
def main(argv: list[str]) -> None:
    ...
    chunk_file_base = os.path.join(tempfile.gettempdir(),'speciesnet_chunks',str(uuid.uuid1()))
    ...
    for i_chunk,instance_chunk in enumerate(instance_chunks):
        ...
        assert instance_paths == set(detection['filepath'] for detection in detection_chunk)
        
        detection_chunk_file = os.path.join(chunk_file_base,'detections_chunk_{}.json'.format(
            str(i_chunk).zfill(4)))
        instance_chunk_file = os.path.join(chunk_file_base,'instances_chunk_{}.json'.format(
            str(i_chunk).zfill(4)))
    ...
    exclude_args = ['--predictions_json','--instances_json','--detections_json',
                    '--filepaths','--filepaths_txt','--folders','--chunk_size']
    all_args = sys.argv
    ...
    for i_chunk in range(0,n_chunks):
        ...
        classification_chunk_file = os.path.join(chunk_file_base,
                                                 'classifications_chunk_{}.json'.format(
                                                     str(i_chunk).zfill(4)))
