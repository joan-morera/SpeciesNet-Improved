...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Measure SpeciesNet model performance for YOLOv10.",
    ...
    parser.add_argument(
        "--model_variant",
        type=str,
        choices=["compact", "extra"],
        required=True,
        help="Specify the YOLOv10 model variant being tested (compact or extra)."
    )
    ...
    try:
        model = SpeciesNet(DEFAULT_MODEL, yolov10_model_name=args.model_variant)
    except Exception as e:
    ...
    model_name_to_report = YOLOV10_MODELS[f"{args.model_variant}_name"]
